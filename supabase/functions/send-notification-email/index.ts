...
serve(async (req) => {
  ...
  try {
    ...
    const toRecipients = Array.isArray(rawTo)
      ? rawTo.filter((value) => typeof value === 'string' && value.trim().length > 0)
      : typeof rawTo === 'string' && rawTo.trim().length > 0
        ? [rawTo.trim()]
        : []
    ...
    if (!RESEND_API_KEY) {
      ...
      return new Response(
        JSON.stringify({
          ...
          data: { to: toRecipients, subject, type, order },
          whatsapp,
        }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
